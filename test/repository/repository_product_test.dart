...
void main() async {
  await Env.i.load();
  final dio = CustomDio();
  final productRepository = ProductRepositoryImpl(dio: dio);

  group('product repository test', () {
    test('should return a list of products when request', () async {
      final list = await productRepository.getAllProducts();
      expect(list[0].title,
          'Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops');
    });

    test('should return a products when request', () async {
      final product = await productRepository.getProduct(1);
      expect(product.title,
          'Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops');
    });

    test('should return a list of products when request filter', () async {
      var data = jsonDecode(jsonData);
      List<ProductModel> listProduct =
          data.map<ProductModel>((p) => ProductModel.fromMap(p)).toList();
      final list = productRepository.runFilter(listProduct, 'casual');
